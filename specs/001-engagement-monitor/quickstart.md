...
scp service-account-key.json pi@<pi-ip>:~/EngageMint/device/config/

# Set environment variable (add to ~/.bashrc for persistence)
export GOOGLE_APPLICATION_CREDENTIALS="$HOME/EngageMint/device/config/service-account-key.json"
```
...
cp model_unquant.tflite ~/EngageMint/device/model/
cp labels.txt ~/EngageMint/device/model/
```
