...
**Structure Decision**: Single project under `EngageMintBackend/` directory at repository root. The frontend (`EngageMintFrontend/`) is a separate, pre-existing project. This device code is independent and deployable to the Pi without any coupling to the frontend build.
