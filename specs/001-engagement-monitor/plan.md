...
**Structure Decision**: Single project under `device/` directory at repository root. The frontend (`EngageMintFrontEnd/`) is a separate, pre-existing project. This device code is independent and deployable to the Pi without any coupling to the frontend build.
